...
public:
 ...
 void input (int **temp_ATA);        //由文件讀入矩陣A的元素以及eps
 void uav ();          //奇异值分解
 void ppp();            //奇异值分解的子程序
 void sss();            //奇异值分解的子程序
 void a_ginv ();         //求A的廣義逆
 void output (double *var_result, double *temp_ATC);        //輸出U矩陣和V矩陣以及奇异值,與A的廣義逆到文件并顯示
...
//int temp_Rvar[MAX_NEIGHBOR_NUMBER];
//int temp_Cvar[MAX_NEIGHBOR_NUMBER];
