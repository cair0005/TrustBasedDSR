...
/*
   In this DSR, node will exchange Hello message every second. In addition,
   this hello message contains one hop neighbors of itself. Passive and active
   blackhole are also implemented in this file
*/
...
static const bool dsragent_prefer_shorter_over_default = false;
static const bool dsragent_always_reestablish = false;
...
void
HelloTimer::expire(Event *)
{
  //a_->update_BWlist_time();
  a_->update_rec_info_time(); 
  a_->update_neighbor_info_time();
  a_->broadcasthello();
  a_->update_sending_history_time();
  a_->update_receiving_history_time();
  resched(HELLO_INTERVAL);
}

//funtion related to pesurdo inverse calculation by CRJ 
void ginv::input(int **temp_ATA)
{
  for(int i=0; i<m; i++)
   for(int j=0; j<n; j++) a[i][j]=temp_ATA[i][j];
...
void ginv::output(double *var_result, double *temp_ATC )
{
  for (int i=0; i<n; i++)
  {
    var_result[i] = 0;
    for (int j=0; j<m; j++)
    {
      var_result[i] = var_result[i] + aa[i][j]*temp_ATC[j];
      //printf("%.4f  ", aa[i][j]);
    }  
  }
}
//end funtion related to pesurdo inverse calculation by CRJ
 
/*void
DSRAgent::collect_BWlist_info()
{  
  for(int i = 0; i < MAX_NEIGHBOR_NUMBER; i++)
  {
    if(b_w_list[i].type == 1 && b_w_list[i].PR_TP == 1)
    {
      w_self[w_self_count]=i;
      w_self_count++;
    }
    if(b_w_list[i].type == 1 && b_w_list[i].PR_TP == 0)
    {
      w_coop[w_coop_count]=i;
      w_coop_count++;
    }
    if(b_w_list[i].type == -1 && b_w_list[i].PR_TP == 1)
    {
      b_self[b_self_count]=i;
      b_self_count++;
    }
    if(b_w_list[i].type == -1 && b_w_list[i].PR_TP == 0)
    {
      b_coop[b_coop_count]=i;
      b_coop_count++;
    }
  }
}
...
void
DSRAgent::clear_BWlist_array()
{
  for(int j = 0; j < MAX_NEIGHBOR_NUMBER; j++)
  {
     w_self[j] = -1;
     w_coop[j] = -1;
     b_self[j] = -1;
...
//help debugging function by CRJ


void
DSRAgent::print_matrix(int **matrix, int row, int col)
{
  for(int i = 0; i < row; i++)
  {
    for(int j = 0; j < col; j++)
...
void
DSRAgent::broadcasthello()
{
  //first, make a new packet
  SRPacket hello_p;

  hello_p.src = net_id;
  hello_p.pkt = allocpkt();

  hdr_sr *srh = hdr_sr::access(hello_p.pkt); 
  hdr_ip *iph = hdr_ip::access(hello_p.pkt);
  hdr_cmn *cmnh =  hdr_cmn::access(hello_p.pkt);
  ...
  srh->init();
  //make it be a hello message
  srh->hello_mark = 1;

  //include all black list and white list
  for(int i=0; i < MAX_NEIGHBOR_NUMBER; i++)
  {
    if(!isBlackhole)
    {
      if(b_w_list[i].black_node == 1 && b_w_list[i].PR_TP == 1)
        srh->hello.rec_info[i] = -1;
      if(b_w_list[i].white_node == 1 && b_w_list[i].PR_TP == 1)  
        srh->hello.rec_info[i] = 1;
    }
    /*else
    {                   
      //if(i >= 22 && i <=41)
      if(b_w_list[i].black_node == 1)
       srh->hello.rec_info[i] = 1;
      if(b_w_list[i].white_node == 1)
       srh->hello.rec_info[i] = -1;   
    }*/    
  }
    
  //if(isBlackhole)
   //printf("this is an attacker\n");

  //other setting in the packet and send it to the link layer
  ...
  cmnh->size() += srh->size();
   ...
   Scheduler::instance().schedule(ll, hello_p.pkt, 0.0);    
}

void
DSRAgent::update_neighbor_info_time()
{
      for(int i = 0; i < MAX_NEIGHBOR_NUMBER; i++)
	if(neighbor_info[i].one_hop_neighbor_id >= 0)
	{
	  neighbor_info[i].time_left--;
	  if(neighbor_info[i].time_left <= 0)
	  {
	    neighbor_info[i].one_hop_neighbor_id = -1;	   
	  }
	}
}

void
DSRAgent::update_rec_info_time()
{
  for(int i = 0; i < MAX_NEIGHBOR_NUMBER; i++)
   if(rec_info[i].time_left > 0)
   {
     rec_info[i].time_left--;
     if(rec_info[i].time_left == 0)
     {
       for(int j = 0; j < MAX_NEIGHBOR_NUMBER; j++)
       {
         rec_info[i].node_type[j] = 0;
       }
     }
   }
}

//extracting neighbor info from the hello message
void
DSRAgent::recv_hello_message(Packet *p)
{
  //hdr_sr *srh = hdr_sr::access(p); 
  hdr_ip *iph = hdr_ip::access(p);

  //whether this hello source can become a new neighbor for us
  int i = 0;
  for(; i < MAX_NEIGHBOR_NUMBER; i++)
  {
    if(neighbor_info[i].one_hop_neighbor_id == iph->saddr())
     break;
  }
  //it is a new neighbor, and we set a new neighbor in our list
  if(i == MAX_NEIGHBOR_NUMBER)
  {
    int l = 0;
    for(; l < MAX_NEIGHBOR_NUMBER; l++)
    {
      if(neighbor_info[l].one_hop_neighbor_id == -1)
       break;
    }
    if(l == MAX_NEIGHBOR_NUMBER)
    {
      printf("note the neighbor list is not enough\n");
      abort();
    }
    neighbor_info[l].one_hop_neighbor_id = iph->saddr();
    neighbor_info[l].time_left = NEIGHBOR_VALID_DURATION;
  }
  else  
     neighbor_info[i].time_left = NEIGHBOR_VALID_DURATION;
 
  /*if(isBlackhole)
  {
    if(iph->saddr()< 22 || iph->saddr()>=42)
     update_whitelist(iph->saddr(), 1);
  }*/
...
void
DSRAgent::update_rec_info(Packet *p)
{
  hdr_sr *srh = hdr_sr::access(p);
  hdr_ip *iph = hdr_ip::access(p);
  int a = iph->saddr(); 
//  int inconsistent_rec_inf[50] = {-1};
//  int index = 0;

  //prepare to update recommeded info
  for(int j = 0; j < MAX_NEIGHBOR_NUMBER; j++)
  {
    rec_info[net_id.getNSAddr_t()].node_type[j] = 0;     
    rec_info[a].node_type[j] = 0;       
  }

  //if(b_w_list[net_id.getNSAddr_t()].white_node != 1)
   //update_whitelist(net_id.getNSAddr_t(), 1);
  ...
  for(; i < MAX_NEIGHBOR_NUMBER; i++)
  {
    if(b_w_list[i].black_node == 1 && b_w_list[i].PR_TP == 1)
    {    
      rec_info[net_id.getNSAddr_t()].node_type[i] = -1;
      rec_info[net_id.getNSAddr_t()].time_left = NEIGHBOR_VALID_DURATION;    
    }
    ...
    if(srh->hello.rec_info[i] == -1)
    {       
      if(rec_info[net_id.getNSAddr_t()].node_type[i] != 1)
      {
        rec_info[a].node_type[i] = -1;
        rec_info[a].time_left = NEIGHBOR_VALID_DURATION;
      }
      else
        break;
    }
  ...
  /*for(int j=0; j < MAX_NEIGHBOR_NUMBER; j++)
  {
    if(rec_info[net_id.getNSAddr_t()].node_type[j] != 0)
    { 
      for(int k=0; k < MAX_NEIGHBOR_NUMBER; k++)
      {
        int diff = rec_info[k].node_type[j]-rec_info[net_id.getNSAddr_t()].node_type[j];
        if(diff == 2 || diff == -2)
        {
          inconsistent_rec_inf[index] = k;
          index++;
        }   
      }
    }
  ...
  {
    int node = inconsistent_rec_inf[a];
    rec_info[node].time_left = 0;
    for(int b = 0; b < MAX_NEIGHBOR_NUMBER; b++)
     rec_info[node].node_type[b] = 0;
  }*/

  update_num_rec_info(); 
  //Generate_matrix_row_file(); 
}

void
DSRAgent::release_memory(int row, int col)
{
  delete [] temp_C; 
  delete [] temp_ATC;
  delete [] var_result;

  for(int i=0; i<num_rec_info.R_var; i++)
  {
    delete [] u_matrix[i];
  }
  delete [] u_matrix;
...
bool
DSRAgent::necessary_compute_bias()
{
  //if(net_id.getNSAddr_t()==11)
   //printf("aaaaaaa\n");  

  if(num_rec_info.rating > num_rec_info.var)
  {
     if(last_rec_info.rating==num_rec_info.rating && last_rec_info.var==num_rec_info.var)
     {
      if(last_rec_info.R_var==num_rec_info.R_var && last_rec_info.C_var==num_rec_info.C_var)
         return false;
      else
      {
         last_rec_info.R_var=num_rec_info.R_var;
         last_rec_info.C_var=num_rec_info.C_var;
         return true;
      } 
     }
     else
     {
        last_rec_info.rating=num_rec_info.rating;
        last_rec_info.var=num_rec_info.var;
        last_rec_info.R_var=num_rec_info.R_var;
        last_rec_info.C_var=num_rec_info.C_var;
        return true;
     }       
...
void
DSRAgent::update_num_rec_info()
{
  
   //prepare to update num_rec_info
   for(int a = 0; a < MAX_NEIGHBOR_NUMBER; a++)
   {     
     num_rec_info.row_var[a] = 0;
     num_rec_info.col_var[a] = 0;
   }

   num_rec_info.var = 0;
   num_rec_info.R_var = 0;
   num_rec_info.C_var = 0;
   num_rec_info.rating = 0;
  //end preparation
   ...
   {     
     for(int j = 0; j < MAX_NEIGHBOR_NUMBER; j++)
     {
       if(rec_info[i].node_type[j] != 0)
       {
         num_rec_info.rating++;
         if(num_rec_info.row_var[i] == 0)
         {
           num_rec_info.row_var[i] = 1;
           num_rec_info.var++;
           num_rec_info.R_var++;
         }
         if(num_rec_info.col_var[j] == 0)
         {
           num_rec_info.col_var[j] = 1;
           num_rec_info.var++;
           num_rec_info.C_var++;
         }         
       }        
   ...
   if(necessary_compute_bias())
   {
     compute_bias();
     //collect_BWlist_info();
     //clear_BWlist_array();
     release_memory(num_rec_info.rating, num_rec_info.var);
     //printf("complete\n");
   }
}

void
DSRAgent::compute_bias()
{
  //initialize the array that store parameters represented in row and colume seperately
  int index_row = 0, index_col = 0;
  for(int i = 0; i < MAX_NEIGHBOR_NUMBER; i++)
  {
     temp_Rvar[i].var = -1;
     temp_Rvar[i].result = 0;
     temp_Cvar[i].var = -1;
     temp_Cvar[i].result = 0;
  }
  
  for(int i = 0; i < MAX_NEIGHBOR_NUMBER; i++)
  {
    if(num_rec_info.row_var[i] == 1)
    {
       temp_Rvar[index_row].var = i;
       index_row ++;
    }
    if(num_rec_info.col_var[i] == 1)
    {
       temp_Cvar[index_col].var = i;
       index_col ++;
    }     
  }
  
  //generate utility matrix
  u_matrix = new int*[index_row];
  for(int i = 0; i < index_row; i++) u_matrix[i] = new int[index_col];
  ...
  //assign value to utility matrix
  for(int i = 0; i < index_row; i++)
  {
    for(int j = 0; j < index_col; j++)
    {
      if(rec_info[temp_Rvar[i].var].node_type[temp_Cvar[j].var] != 0)
      {
        u_matrix[i][j] = rec_info[temp_Rvar[i].var].node_type[temp_Cvar[j].var];
      }
    }
  }

  //generate targe matrix A and C  
  temp_C = new int[num_rec_info.rating];
  temp_A = new int*[num_rec_info.rating];
  for (int i=0; i<num_rec_info.rating; i++) temp_A[i] = new int[num_rec_info.var];
  
  for(int i = 0; i < num_rec_info.rating; i++)
  {
   temp_C[i] = 0;
   for(int j = 0; j < num_rec_info.var; j++)  temp_A[i][j] = 0;
  }

  //assign value to matrix A and C
  int count = -1;    
  for(int j = 0; j < num_rec_info.C_var; j++)
  {
    for(int i = 0; i < num_rec_info.R_var; i++)
    {
      if(rec_info[temp_Rvar[i].var].node_type[temp_Cvar[j].var] != 0)
      {
        count = count+1; 
        temp_A[count][i] = 1;
        temp_A[count][j+num_rec_info.R_var] = 1;
        temp_C[count] = rec_info[temp_Rvar[i].var].node_type[temp_Cvar[j].var];
      }
    }
  }

  if((count+1) != num_rec_info.rating)
  {
   printf("num_rec_info.rating is wrong\n");
   abort();   
  }
  else
...
void 
DSRAgent::matrix_multiplication()
{
  //creat transpose matrix A
  temp_AT = new int*[num_rec_info.var];
  for (int i=0; i<num_rec_info.var; i++) temp_AT[i] = new int[num_rec_info.rating];

  for(int i = 0; i < num_rec_info.rating; i++)
   for(int j = 0; j < num_rec_info.var; j++)  temp_AT[j][i] = temp_A[i][j];

  //multiply [transpose A]*[A] and store result in temp_ATA
  temp_ATA = new int*[num_rec_info.var];
  for (int i = 0; i<num_rec_info.var; i++) temp_ATA[i] = new int[num_rec_info.var];

  for(int i = 0; i < num_rec_info.var; i++)
  {
    for(int j = 0; j < num_rec_info.var; j++)
    {
      temp_ATA[i][j] = 0;
      for(int k = 0; k < num_rec_info.rating; k++)
       temp_ATA[i][j] = temp_ATA[i][j]+temp_AT[i][k]*temp_A[k][j];
    }
  }

  //multiply [transpose A]*[C] and store result in temp_ATC
  temp_ATC = new double[num_rec_info.var];
  for(int i = 0; i < num_rec_info.var; i++)  
  {
    temp_ATC[i] = 0;
    for(int j = 0; j < num_rec_info.rating; j++)
    temp_ATC[i]=temp_ATC[i]+temp_AT[i][j]*temp_C[j];
  }

  var_result = new double[num_rec_info.var];
  for(int i = 0; i < num_rec_info.var; i++)  var_result[i] = 0;
  
  ginv uv(num_rec_info.var, num_rec_info.var);
  uv.input(temp_ATA);
  uv.a_ginv();
  uv.output(var_result, temp_ATC);

  compute_recommended_trust(var_result);  
}

void
DSRAgent::compute_recommended_trust(double *var_result)
{
  int r_index=0, net_id_index;
  double rec_result;

  for(int i=0; i<num_rec_info.R_var; i++)
  {
    temp_Rvar[i].result = var_result[i];
    if(temp_Rvar[i].var == net_id.getNSAddr_t())
     net_id_index = i;
  }

  for(int j=num_rec_info.R_var; j<num_rec_info.var; j++)
  {
    temp_Cvar[r_index].result = var_result[j];
    r_index++;
  }
  
  if(r_index!=num_rec_info.C_var)
  {
   printf("num_rec_info.C_var is wrong\n");
   abort();
  }
  else
   r_index=0;

  if(net_id_index < 0 || net_id_index >= num_rec_info.R_var )
  {
    for(int i=0; i<num_rec_info.C_var; i++)
    {
      rec_result = temp_Cvar[i].result;
      if(rec_result >= REC_WHITE_THRESHOLD && temp_Cvar[i].var != net_id.getNSAddr_t())
       update_whitelist(temp_Cvar[i].var, 0);
      if(rec_result <= REC_BLACK_THRESHOLD)
       update_blacklist(temp_Cvar[i].var, 0 && temp_Cvar[i].var != net_id.getNSAddr_t());
    }
  }
  else
  {
    for(int i=0; i<num_rec_info.C_var; i++)
    {
      rec_result = temp_Rvar[net_id_index].result + temp_Cvar[i].result;
      if(rec_result >= REC_WHITE_THRESHOLD && temp_Cvar[i].var != net_id.getNSAddr_t())
       update_whitelist(temp_Cvar[i].var, 0);
      if(rec_result <= REC_BLACK_THRESHOLD && temp_Cvar[i].var != net_id.getNSAddr_t())
       update_blacklist(temp_Cvar[i].var, 0);
    }
  }
}
...
send_buf_timer(this), hello_brdcst(this), flow_table(), ars_table()
{
  int c;
  route_request_num = 1;
  ...
  bind("isBlackhole", &isBlackhole);
  bind("isActive", &isActive);

//INITIALIZATION OF THE NEIGHBOR/BLACKLIST/SUSPICIOUS list by CRJ 
  for(int i = 0; i < MAX_NEIGHBOR_NUMBER; i++)
  {
        //blacklist[i].node_id = -1;
        //blacklist[i].time_left = 0;
        b_w_list[i].black_node = 0;
        b_w_list[i].white_node = 0;
        b_w_list[i].Btime_left = 0;
        b_w_list[i].Wtime_left = 0;
        b_w_list[i].type = 0;
        b_w_list[i].PR_TP = 0;

        trust_level[i].node_id = -1;
        trust_level[i].trust = 0;
        trust_level[i].distrust = 0;

        neighbor_info[i].one_hop_neighbor_id = -1;
	neighbor_info[i].time_left = 0;
        
        num_rec_info.row_var[i] = 0;
        num_rec_info.col_var[i] = 0;
   ...
   last_rec_info.var = 0;
   last_rec_info.R_var = 0;
   last_rec_info.C_var = 0;
   last_rec_info.rating = 0;
...
//INITIALIZATION OF THE send/receive history 
  for(int i=0; i < MAX_HISTORY_RECORD; i++)
  {
     send_history[i].dest_id = -1;
     send_history[i].total_packet = 0;
     send_history[i].time_left = 0;
     receive_history[i].time_left = 0;
     receive_history[i].total_packet = 0;
     receive_history[i].src_id = -1;
     for(int j = 0; j < MAX_ROUTE_LEN; j++)
     {
       send_history[i].sr_route[j] = -1;
       receive_history[i].sr_route[j] = -1;
     }
     for(int k = 0; k < MAX_NEIGHBOR_NUMBER; k++)
     {
       receive_history[i].rep_rec[k].node = -1;
       receive_history[i].rep_rec[k].rep_pkt = 0;
     }

  }
//End initialization by CRJ
   ...
   the MAC broadcast addr */
{
  ...
  if (srh->valid() != 1) {
    ...
    {   
      //Added by CRJ  
      if(srh->invest_req_mark == 1 && net_id == p.dest)
      {
        int num=0;  
        for(; num < MAX_INVEST_RECORD; num++)
        { 
          if(srh->invest_req[num].route_len == 0)
          break;
        }
        //printf("receive investigation req");
        send_investigation_reply(p, num);
      }

      if(srh->invest_rep_mark == 1 && net_id == p.dest)
      {    
         //printf("receive investigation rep");
         trust_evaluation(p);
      } 

      if(srh->hello_mark == 1)
      {	
        recv_hello_message(packet);
        send_investigation_request(p);
	Packet::free(p.pkt);
	p.pkt = 0;
	return;
      }
	...
	{ // propagate a route_request that's not for us
	  printf("the current node is %d , blackhole: %d \n", net_id.getNSAddr_t(), isBlackhole);
...
----------------------------------------------------------------------------*/
/*void
DSRAgent::update_BWlist_time()
{
      for(int i = 0; i < MAX_NEIGHBOR_NUMBER; i++)
      {
	if(b_w_list[i].black_node == 1 && b_w_list[i].PR_TP == 0)
	{
	  b_w_list[i].Btime_left--;
	  if(b_w_list[i].Btime_left == 0) {
	    b_w_list[i].black_node = 0;
            b_w_list[i].type = 0; }	        
	}
        if(b_w_list[i].white_node == 1 && b_w_list[i].PR_TP == 0)
        {
          b_w_list[i].Wtime_left--;
          if(b_w_list[i].Wtime_left == 0) {
           b_w_list[i].white_node = 0;
           b_w_list[i].type = 0; }
        }                              
      } 
...
void
DSRAgent::update_whitelist(int node_id, int PR_TP)
{    
   /*if(!isBlackhole)
   {     
     //if(node_id < 20 && PR_TP == 0)
     if(node_id < 11 || node_id >= 42)
     {
       if(b_w_list[node_id].type == -1 && b_w_list[node_id].PR_TP == 0)
        printf("whitelist update is correct\n");
     }
   }*/

   if(PR_TP >= b_w_list[node_id].PR_TP)
   {     
     b_w_list[node_id].black_node = 0;
     b_w_list[node_id].white_node = 1;
     b_w_list[node_id].Btime_left = 0;
     b_w_list[node_id].type = 1;
     b_w_list[node_id].PR_TP = PR_TP;
     /*if(PR_TP >=1)
      b_w_list[node_id].Wtime_left = WHITE_DURATION;
     else
      b_w_list[node_id].Wtime_left = TEMP_WHITE_DURATION;*/
   }
}

void
DSRAgent::update_blacklist(int node_id, int PR_TP)
{
   /*if(!isBlackhole)
   {
     if(node_id >= 22 && node_id < 42)
     {
       if(b_w_list[node_id].type == 1 && b_w_list[node_id].PR_TP == 0)
       //if(PR_TP == 0)
        printf("blacklist is correct\n");
     }
   }*/   

   if(PR_TP >= b_w_list[node_id].PR_TP)  
   {
     b_w_list[node_id].black_node = 1;
     b_w_list[node_id].white_node = 0;
     b_w_list[node_id].Wtime_left = 0;    
     b_w_list[node_id].type = -1;
     b_w_list[node_id].PR_TP = PR_TP;
     /*if(PR_TP >= 1)
       b_w_list[node_id].Btime_left = BLACK_DURATION;
     else
       b_w_list[node_id].Btime_left = TEMP_BLACK_DURATION;*/
   }   
}

bool 
DSRAgent::in_blacklist(int node_id)
{
     if(b_w_list[node_id].black_node == 1)
      return true;
     else
      return false;
}
/*===========================================================================
  handlers for trust evaluation by CRJ
----------------------------------------------------------------------------*/
void
DSRAgent::clear_sending_history(int row_index)
{
  send_history[row_index].dest_id = -1;
  send_history[row_index].total_packet = 0;
  send_history[row_index].time_left = 0;
  for(int i = 0; i < MAX_ROUTE_LEN; i++)
   send_history[row_index].sr_route[i] = -1;
}

void
DSRAgent::trust_evaluation(SRPacket &p)
{
  hdr_sr *srh =  hdr_sr::access(p.pkt);

  for(int i = 0; i < MAX_INVEST_RECORD; i++)
  {   
    if(srh->invest_rep[i].total_packet == 0)
    {
       int j = 0;
       for(; j < MAX_ROUTE_LEN; j++)
       {
         if(net_id.getNSAddr_t()==srh->invest_rep[i].sr_route[j])
          break;
       }
       if(j != (srh->invest_rep[i].route_len-2) && j != (srh->invest_rep[i].route_len-1))  
       {
         int index_SHT_A = corresponding_row_SHT(p, i);
         if(index_SHT_A != -1)
         {
           int lost_Pkt = send_history[index_SHT_A].total_packet;                     
           increase_distrust(index_SHT_A, j, lost_Pkt);
           clear_sending_history(index_SHT_A);           
         }
       }    
    }
    else if(srh->invest_rep[i].total_packet != -1)
    {
       int k = 0;
       for(; k < MAX_ROUTE_LEN; k++)
       {
         if(net_id.getNSAddr_t()==srh->invest_rep[i].sr_route[k])
          break;
       }
       if(k != (srh->invest_rep[i].route_len-2) && k != (srh->invest_rep[i].route_len-1))  
       {
         int index_SHT_B = corresponding_row_SHT(p, i);
         if(index_SHT_B != -1)
         {
           int total_send = send_history[index_SHT_B].total_packet;
           int total_received = srh->invest_rep[i].total_packet;
           float PDR = (float)total_received/(float)total_send;

           if(PDR < PDR_THRESHOLD)
           {
             int lost_Pkt_B = total_send-total_received;
             increase_distrust(index_SHT_B, k, lost_Pkt_B);
             clear_sending_history(index_SHT_B);
           }
           else
           {
             increase_trust(index_SHT_B, k, total_received);
             clear_sending_history(index_SHT_B);
           }
         }
       }
...
int 
DSRAgent::corresponding_row_SHT(SRPacket &p, int row_IREP)
{
  hdr_sr *srh =  hdr_sr::access(p.pkt);

  int len = srh->invest_rep[row_IREP].route_len;
  int dest_id = srh->invest_rep[row_IREP].sr_route[len-1];
  int i = 0;

  for(; i < MAX_HISTORY_RECORD; i++)
  {
    if(dest_id == send_history[i].dest_id )
    {
       int j = 0;
       for(; j < len; j++)
       {
         if(srh->invest_rep[row_IREP].sr_route[j] != send_history[i].sr_route[j]) 
           break;
       }
       if(j == len && send_history[i].sr_route[j] == -1)
        return i;
    }
  }
  if(i == MAX_HISTORY_RECORD)
   return -1;
}

int
DSRAgent::exist_in_TET(int node_id)
{
  if(trust_level[node_id].node_id == node_id)
   return 1;
  else
   return 0;
}
...
void
DSRAgent::increase_distrust(int index_SHT, int investigator_pos, int lost_Packet)
{  
   if(lost_Packet <= 1)
   {
      printf("lost pkt less than one\n");
   }
   if(net_id.getNSAddr_t()!= send_history[index_SHT].sr_route[investigator_pos])
   {
   ...
   {   
     if(send_history[index_SHT].sr_route[i] == -1)
      break;
   }

   int des_pos = i-1;
   int pos = investigator_pos+1;
   float distrust_weight;

   for(; pos < des_pos; pos++)
   {    
       int k = send_history[index_SHT].sr_route[pos];
       trust_level[k].node_id = send_history[index_SHT].sr_route[pos];
       int interval = des_pos-investigator_pos-1;
       distrust_weight = (float)lost_Packet/interval;
       trust_level[k].distrust = trust_level[k].distrust + distrust_weight; 

       float total_event = trust_level[k].distrust + trust_level[k].trust + 2;
       float uncertainty = 2/total_event;
       float distrust = (trust_level[k].distrust/total_event) * (1-uncertainty);      
         
       if(distrust > CONFIDENT_THRESHOLD)                         
        update_blacklist(trust_level[k].node_id, 1);              
   }  
}

void
DSRAgent::increase_trust(int index_SHT, int investigator_pos, int received_Packet)
{     
   //int trust_weight;
   if(net_id.getNSAddr_t()!= send_history[index_SHT].sr_route[investigator_pos])
   {
   ...
   //trust_weight = send_history[index_SHT].delt_send/INVEST_THRESHOLD;
   ...
   {   
       int k = send_history[index_SHT].sr_route[pos];   
       trust_level[k].node_id = send_history[index_SHT].sr_route[pos];   
       trust_level[k].trust = trust_level[k].trust + received_Packet;
   
       float total_event = trust_level[k].distrust + trust_level[k].trust + 2;
       float uncertainty = 2/total_event;
       float trust = (trust_level[k].trust/total_event) * (1-uncertainty);
     
       if(trust > CONFIDENT_THRESHOLD)      
       {                  
         update_whitelist(trust_level[k].node_id, 1);
       }                                     
   } 
}

void
DSRAgent::increase_trust_prev(SRPacket &p)
{
  /*hdr_sr *srh = hdr_sr::access(packet);
  hdr_ip *iph = hdr_ip::access(packet);
  hdr_cmn *cmh = hdr_cmn::access(packet);*/

  int pos = p.route.index()-1;
  for(; pos > 0; pos--)
  {
    int k = p.route[pos].getNSAddr_t();
    trust_level[k].node_id = k;
    trust_level[k].trust++;
...
/*===========================================================================
  investigation functions by CRJ
-----------------------------------------------------------------------------*/
void
DSRAgent::send_investigation_request(SRPacket &p)
{
  int invest_row_SHT[MAX_INVEST_RECORD];//Store the row of SHT that need to generate a invest req
  for(int i = 0; i < MAX_INVEST_RECORD; i++)//initialization
   invest_row_SHT[i] = -1;
 
 //extracted the row of SHT that need to generate a invest req 
  int index = 0;  
  for(int i = 0; i < MAX_HISTORY_RECORD; i++)
  { 
    if(send_history[i].total_packet >= 10)
    {
      if(p.src.getNSAddr_t() == send_history[i].dest_id)
      {      
       invest_row_SHT[index]=i;
       index++;            
      }
    }
  }

  if(index >= MAX_INVEST_RECORD)
  {
    printf("investigation record is not enough\n");
    //abort();
  } 
  if(index == 0) // No need to initiate any invest req
   return;
  else //generate an investigation request
  {
     SRPacket  investReq_p;
     
     investReq_p.pkt = allocpkt();
     investReq_p.src = net_id;
     investReq_p.dest = p.src;

     hdr_sr *srh =  hdr_sr::access(investReq_p.pkt);
     hdr_ip *iph =  hdr_ip::access(investReq_p.pkt);
     hdr_cmn *cmnh =  hdr_cmn::access(investReq_p.pkt);

     iph->daddr() = Address::instance().create_ipaddr(investReq_p.dest.getNSAddr_t(),RT_PORT);
     iph->dport() = RT_PORT;
     iph->saddr() = Address::instance().create_ipaddr(net_id.getNSAddr_t(),RT_PORT);
     iph->sport() = RT_PORT;
     cmnh->ptype() = PT_DSR;
     ...
     srh->init();
     srh->invest_req_mark = 1;//make it to be an invest req msg

     //include all srRoutes that need investigation
     int num = 0;
     if(index > MAX_INVEST_RECORD)
      index = MAX_INVEST_RECORD;
     for(int k = 0; k < index; k++)
     {
       int extract_row = invest_row_SHT[k], l = 0;
       for(; l < MAX_ROUTE_LEN; l++) 
       {
         srh->invest_req[num].sr_route[l] = send_history[extract_row].sr_route[l];
         if(send_history[extract_row].sr_route[l] == -1)
          break;
       }
       srh->invest_req[num].route_len = l;
       srh->invest_req[num].total_packet = send_history[extract_row].total_packet;
       num++;
     }
     ...
     cmnh->xmit_failure_ = 0;
     cmnh->prev_hop_ = net_id.getNSAddr_t();
     cmnh->next_hop() = investReq_p.dest.getNSAddr_t();
     cmnh->addr_type() = NS_AF_ILINK;
 
     Scheduler::instance().schedule(ll, investReq_p.pkt, 0.0);     
  }
}

void
DSRAgent::send_investigation_reply(SRPacket &p, int num_of_record)
{
  hdr_sr *old_srh =  hdr_sr::access(p.pkt);

  int related_row_RHT[MAX_INVEST_RECORD];//Store the row of RHT that need to generate a invest req
  for(int i = 0; i < MAX_INVEST_RECORD; i++)//initialization
   related_row_RHT[i] = -1;   

  //extracted the row of RHT that contain requested informatin
  int index = 0; 
  for(int i=0; i < num_of_record; i++)
  {     
    int j = 0;
    for(; j < MAX_HISTORY_RECORD; j++)
    {
      if(old_srh->invest_req[i].sr_route[0] == receive_history[j].src_id)
      {
        int k = 0;
        for(; k < old_srh->invest_req[i].route_len; k++)           
        {
          if(old_srh->invest_req[i].sr_route[k] != receive_history[j].sr_route[k])
           break;
        }
        if(k == old_srh->invest_req[i].route_len)
        {
          related_row_RHT[index] = j;
          index++;
          break;
        }
      }      
    }
    if(j == MAX_HISTORY_RECORD)
    {
     related_row_RHT[index] = -2;
     index++;
    }          
  }
  assert(index==num_of_record);
  
  SRPacket  investRep_p;    
  investRep_p.pkt = allocpkt();
  investRep_p.src = net_id;
  investRep_p.dest = p.src;

  hdr_sr *srh =  hdr_sr::access(investRep_p.pkt);
  hdr_ip *iph =  hdr_ip::access(investRep_p.pkt);
  hdr_cmn *cmnh =  hdr_cmn::access(investRep_p.pkt);

  iph->daddr() = Address::instance().create_ipaddr(investRep_p.dest.getNSAddr_t(),RT_PORT);
  iph->dport() = RT_PORT;
  iph->saddr() = Address::instance().create_ipaddr(net_id.getNSAddr_t(),RT_PORT);
  iph->sport() = RT_PORT;
  cmnh->ptype() = PT_DSR;
  ...
  srh->init();
  srh->invest_rep_mark = 1;

  //reply all routes contain in request msg
  int num = 0;
  for(int k = 0; k < num_of_record; k++)
  {
    int extract_row = related_row_RHT[k], l = 0;
    if(extract_row == -2)
    {  
      for(int m=0; m < MAX_ROUTE_LEN; m++)
      {
        srh->invest_rep[num].sr_route[m] = old_srh->invest_req[num].sr_route[m];  
      }  
      srh->invest_rep[num].total_packet = 0;
      srh->invest_rep[num].route_len = old_srh->invest_req[num].route_len;
      num++;
    }
    else
    {
      for(; l < MAX_ROUTE_LEN; l++) 
      {
        srh->invest_rep[num].sr_route[l] = receive_history[extract_row].sr_route[l];
        if(receive_history[extract_row].sr_route[l] == -1)
         break;
      }

      int node = investRep_p.dest.getNSAddr_t();
      receive_history[extract_row].rep_rec[node].node = node;
      int total_rec_pkt = receive_history[extract_row].total_packet;
      int last_rep_pkt = receive_history[extract_row].rep_rec[node].rep_pkt;
      receive_history[extract_row].rep_rec[node].rep_pkt = total_rec_pkt;

      srh->invest_rep[num].total_packet = total_rec_pkt-last_rep_pkt;
      srh->invest_rep[num].route_len = old_srh->invest_req[num].route_len;
      num++;
    }
  }
  ...
  cmnh->xmit_failure_ = 0;
  cmnh->prev_hop_ = net_id.getNSAddr_t();
  cmnh->next_hop() = investRep_p.dest.getNSAddr_t();
  cmnh->addr_type() = NS_AF_ILINK;
 
  Scheduler::instance().schedule(ll, investRep_p.pkt, 0.0); 
}
/*===========================================================================
  handlers for sending history table by CRJ
---------------------------------------------------------------------------*/
void
DSRAgent::update_sending_history_time()
{
      for(int i = 0; i < MAX_HISTORY_RECORD; i++)
	if(send_history[i].dest_id >= 0)
	{
	  send_history[i].time_left--;
	  if(send_history[i].time_left <= 0)
	  {
	   send_history[i].dest_id = -1;
           send_history[i].total_packet = 0;
	   for(int k = 0; k < MAX_ROUTE_LEN; k++)
	    send_history[i].sr_route[k] = -1;
	  }
	}
}

int
DSRAgent::exist_in_SHT(SRPacket &p)
{
    int row_SRroute[MAX_HISTORY_RECORD]; 
    for(int a = 0; a < MAX_HISTORY_RECORD; a++) //initialization
      row_SRroute[a] = -1; 

    //sr_route of correspond row in SHT need to be checked
    int index = 0, stalest_index=-1, stalest=900;
    for(int i = 0; i < MAX_HISTORY_RECORD; i++)
    {
      //find the stalest record in sending history table
      if(send_history[i].dest_id >= 0 && send_history[i].time_left < stalest)
      {
         stalest = send_history[i].time_left;
         stalest_index = i; 
      } // function end
      
      if(send_history[i].dest_id == p.dest.getNSAddr_t())
      {
        row_SRroute[index] = i;
        index++;
      }    
    ...
    if(index == 0) //completely new CBR 
    {
      int j = 0;
      for(; j < MAX_HISTORY_RECORD; j++)
      {
        if(send_history[j].dest_id == -1)
         return j;
      }
      if(j == MAX_HISTORY_RECORD)
      {
        printf("SHT is not enough\n");

        if(stalest_index>=0)
        {
          send_history[stalest_index].dest_id = -1;
          send_history[stalest_index].total_packet = 0;
          send_history[stalest_index].time_left = 0;
          for(int a = 0; a < MAX_ROUTE_LEN; a++)
           send_history[stalest_index].sr_route[a] = -1;
          return stalest_index;
        }        
	//abort();
      } 
    }
    else //dest id of CBR exits in SHT, check whether the sr_route of CBR also exist
    {     
      for(int k = 0; k < index; k++)
      {
         int checking_row = row_SRroute[k], l=0;
         for(; l < MAX_ROUTE_LEN; l++)
         {
            if(send_history[checking_row].sr_route[l] != p.route[l].getNSAddr_t())
             break;
         }
         if(l == p.route.length())
         { 
           if(send_history[checking_row].sr_route[l] == -1||l == MAX_ROUTE_LEN) 
            return checking_row; //sr_route also exist in SHT           
         }    
      }
  
     //p.dest exist in SHT, sr_route is not exist. Need to creat a new record
      int m=0;
      for(; m < MAX_HISTORY_RECORD; m++)
      {
        if(send_history[m].dest_id == -1)
         return m;
      }
      if(m == MAX_HISTORY_RECORD)
      {
        printf("SHT is not enough\n");
...
void
DSRAgent::update_sending_history(SRPacket &p)
{ 
  int index = exist_in_SHT(p); 
  if (send_history[index].dest_id == -1)
  {
     send_history[index].dest_id = p.dest.getNSAddr_t();
     send_history[index].total_packet++;
     send_history[index].time_left = HISTORY_VALID_DURATION;
     
     for(int j = 0; j < MAX_ROUTE_LEN; j++)
      send_history[index].sr_route[j] = -1;//ensure sr_route of this index in SHT is empty  

     for(int i=0; i < p.route.length(); i++)     
      send_history[index].sr_route[i] = p.route[i].getNSAddr_t();
...
/*===========================================================================
  handlers for receiving history table by CRJ
---------------------------------------------------------------------------*/
void
DSRAgent::update_receiving_history_time()
{
      for(int i = 0; i < MAX_HISTORY_RECORD; i++)
	if(receive_history[i].src_id >= 0)
	{
	  receive_history[i].time_left--;
	  if(receive_history[i].time_left <= 0)
	  {
           receive_history[i].src_id = -1;
           receive_history[i].total_packet = 0;
	   for(int k = 0; k < MAX_ROUTE_LEN; k++)
	    receive_history[i].sr_route[k] = -1;
	  }
	}
}

int
DSRAgent::exist_in_RHT(SRPacket &p)
{
    int row_SRroute[MAX_HISTORY_RECORD];
    for(int a = 0; a < MAX_HISTORY_RECORD; a++) //initialization
      row_SRroute[a] = -1; 

   //sr_route of correspond row in RHT need to be checked 
    int index = 0, stalest_index=-1, stalest=900;
    for(int i = 0; i < MAX_HISTORY_RECORD; i++)
    {
      //find the stalest record in sending history table
      if(receive_history[i].src_id >= 0 && receive_history[i].time_left < stalest)
      {
         stalest = receive_history[i].time_left;
         stalest_index = i; 
      } // function end

      if(receive_history[i].src_id == p.src.getNSAddr_t())
      {
        row_SRroute[index] = i;
        index++;
      }    
    ...
    if(index == 0) //completely new CBR 
    {
      ...
      for(; j < MAX_HISTORY_RECORD; j++)
      {
        if(receive_history[j].src_id == -1)
         return j;
      }
      if(j == MAX_HISTORY_RECORD)
      {
        printf("RHT is not enough\n");
	//abort();
        
        if(stalest_index>=0)
        {
          receive_history[stalest_index].src_id = -1;
          receive_history[stalest_index].total_packet = 0;
          receive_history[stalest_index].time_left = 0;
          for(int a = 0; a < MAX_ROUTE_LEN; a++)
           receive_history[stalest_index].sr_route[a] = -1;
          for(int b = 0; b < MAX_NEIGHBOR_NUMBER; b++)
          {
            receive_history[stalest_index].rep_rec[b].node = -1;
            receive_history[stalest_index].rep_rec[b].rep_pkt = 0;
          }
          return stalest_index;
        }        
      } 
    }
    else //src id of CBR exits in RHT, check whether the sr_route of CBR also exist
    {     
      for(int k = 0; k < index; k++)
      {
         int checking_row = row_SRroute[k], l=0;
         for(; l < MAX_ROUTE_LEN; l++)
         {
            if(receive_history[checking_row].sr_route[l] != p.route[l].getNSAddr_t())
             break;
         }
         if(l == p.route.length())
         {
           if(receive_history[checking_row].sr_route[l] == -1 || l == MAX_ROUTE_LEN) 
            return checking_row; //sr_route also exist in SHT           
         }    
      }
  
     //p.src exist in SHT, sr_route is not exist. Need to creat a new record
      int m=0;
      for(; m < MAX_HISTORY_RECORD; m++)
      {
        if(receive_history[m].src_id == -1)
         return m;
      }
      if(m == MAX_HISTORY_RECORD)
      {
	...
	//abort();
        ...
        if(stalest_index>=0)
        {
          ...
          receive_history[stalest_index].total_packet = 0;
          receive_history[stalest_index].time_left = 0;
          for(int c = 0; c < MAX_ROUTE_LEN; c++)
           send_history[stalest_index].sr_route[c] = -1;
          for(int d = 0; d < MAX_NEIGHBOR_NUMBER; d++)
          {
            receive_history[stalest_index].rep_rec[d].node = -1;
            receive_history[stalest_index].rep_rec[d].rep_pkt = 0;
          }
          return stalest_index;
        }
      } 
...
void
DSRAgent::update_receiving_history(SRPacket &p)
{  
  int index = exist_in_RHT(p); 
  if (receive_history[index].src_id == -1)
  {
     receive_history[index].src_id = p.src.getNSAddr_t();
     receive_history[index].total_packet++;
     receive_history[index].time_left = HISTORY_VALID_DURATION;
     
     for(int j = 0; j < MAX_ROUTE_LEN; j++)
      receive_history[index].sr_route[j] = -1;//ensure sr_route of this index in SHT is empty  

     for(int i=0; i < p.route.length(); i++)
     {
       receive_history[index].sr_route[i] = p.route[i].getNSAddr_t();
     }
  }
  else
  {
...
//dest updates receiving history table by CRJ 
  if(cmh->ptype() == PT_CBR && p.dest == net_id)
  {                
    update_receiving_history(p);
    //printf("Finish update RHT\n");
  }
  ...
  if (srh->route_reply())
    { // we got a route_reply piggybacked on a route_request
      ...
      // (we'll send off any packet's we have queued and waiting)
         int next_hop = cmh->prev_hop_;  
     if (!in_blacklist(next_hop))
      {               
           acceptRouteReply(p);   //only this funtion call in original DSR
      }
      else
      {
         drop(p.pkt, DROP_RTR_DETECT);
         return;
      }
...
DSRAgent::handleFlowForwarding(SRPacket &p, int flowidx) {
  ...
  if (!srh->salvaged() && 
    ...
    p.route.index() -= amt;
    sendRouteShortening(p, p.route.index(), flow_table[flowidx].sourceRoute.index());
  }
...
bool
DSRAgent::disturbForwarding(SRPacket &p)
{
  hdr_sr *srh =  hdr_sr::access(p.pkt);
  hdr_cmn *cmnh = hdr_cmn::access(p.pkt);
  ...
  if(isBlackhole && type == 0)
  {
     if(srh->route_reply() || srh->route_request())
     {
        //pretend to be a good node
        return false;
     }
     else 
     {                      
        if(cmnh->ptype() == PT_CBR && p.dest != net_id)
        {                        
           if(p.route.index() >= 2 && p.route.length()>2)
            increase_trust_prev(p);         
        }
        return true;
      
     }
  }
  else if(isBlackhole && type == 1)
  {
     int ran_num ;
     srand((unsigned)time(NULL));
     ran_num=rand() % 3;
     ...
     {
       if(ran_num !=0)        
        return true;  //but drop all other packets
       else
        return false;      
     }
  }
   ...
   snooping as appropriate */
{
  ...
  if (disturbForwarding(p))
  {
       drop(p.pkt, DROP_RTR_ATTACK);
       return;
  }

   //check forwading path. Added by CRJ 
  if (!isBlackhole )
  {
     if (srh->route_reply())
     { 
        if(p.route.index() == 1)
        {
          if(in_blacklist(p.route[2].getNSAddr_t()) && p.route.length() > 3)
          {
           drop(p.pkt, DROP_RTR_DETECT);
           return;
          }
        } 
        if(p.route.index() > 1 && p.route.index() < p.route.length()-2)
        {
           int pre_id = p.route[p.route.index()-1].getNSAddr_t();
           int next_id = p.route[p.route.index()+1].getNSAddr_t();
           if(in_blacklist(pre_id) || in_blacklist(next_id))
           {
             drop(p.pkt, DROP_RTR_DETECT);
             return;
           }
        }
        if (p.route.index() == p.route.length()-2 && p.route.length() >= 3)
        {        
           if(in_blacklist(p.route[p.route.length()-3].getNSAddr_t()))
           {
             drop(p.pkt, DROP_RTR_DETECT);
             return;
           }
     ...
     if(ch->ptype() == PT_CBR)
     {
       int next = p.route[p.route.index()+1].getNSAddr_t();       
       if (in_blacklist(next) && p.route.index() < p.route.length()-2)
       {    
          xmitFailed(p.pkt, DROP_RTR_DETECT);
          return;
       }
     }
  }  
 //end check forwarding path.  
  ...
  /* process a route request that isn't targeted at us */
{
  ...
  //Active Blackhole nodes reply all route request packets in advance
     ...
     //  set as it currently is
{
 ...
 //source and intermediate nodes update sending history table by CRJ
  if(cmnh->ptype() == PT_CBR && p.dest != net_id)
  {                
    if(p.route.index() != (p.route.length()-2) && p.route.length() > 2)
    {
      update_sending_history(p);
      if(p.route.index() >= 2)
      {
        increase_trust_prev(p);
      } 
  ...
  if (dsragent_enable_flowstate &&
    ...
    cmnh->size() += srh->size();
     ...
     //added by CRJ
    if(cmnh->ptype() == PT_CBR && !isBlackhole)
    {
      //checkSrcRoute(p);
      int checked_node = srh->get_next_addr();
      if(in_blacklist(checked_node)) {
       xmitFailed(p.pkt, DROP_RTR_DETECT);
       p.pkt = 0;
       return; }
    }
   //end CRJ
	...
	cmnh->prev_hop_ = net_id.getNSAddr_t(); //Add by CRJ
  ...
  if (srh->route_request())
    { // route requests need to be jittered a bit
      Scheduler::instance().schedule(ll, p.pkt, Random::uniform(RREQ_JITTER) + delay);
    }
  else
    { // no jitter required 
     ...
     in which case it is not touched */
{
  ...
  update_whitelist(p.dest.getNSAddr_t(), 1);
  ...
  // doesn't free p.pkt
{
  ...
  if (isActive && isBlackhole) { 
    if(net_id != p.dest)  
    {
      new_route.reverseInPlace();
      route_cache->addRoute(new_route, Scheduler::instance().clock(), net_id);
    }
    else
  ...
  {
    if(net_id != p.dest)
     new_srh->cur_addr() = new_srh->cur_addr() + 1;
  }
     ...
     p.pkt freed or handed off */
{
  ...
  if(strcmp(reason, "DET") != 0) //this if is added by CRJ
